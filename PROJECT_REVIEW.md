...
- [x] **`cook_private.permit_photo_path` is owner-writable** — ✅ FIXED Aug 2 2026 (migration 38 `harden-cook-private.sql` + the wizard's permit-photo write moved to the service role). A `before insert/update` trigger `enforce_cook_private_rules` now lets an end-user session write only `street_address`/`cdtfa_permit`/`pickup_location`/`updated_at`; `permit_photo_path` + `latitude`/`longitude` are server-managed, so a cook can no longer repoint the photo path at another cook's storage object.
...
- [ ] **Dish allergens — Phase 2 (buyer-side filter/warning)** — 🅵 DEFERRED (Aug 2 2026). Phase 1 shipped (PR #79): structured per-dish `contains` / `may_contain` + a required `allergens_declared`, taxonomy in `lib/allergens.ts`. Phase 2 is the actual differentiator — let a buyer say "I'm allergic to shellfish" and flag/hide matching dishes across browse + kitchen pages. It needs the structured data Phase 1 now captures, so build once there's real inventory to filter. Keep the same non-negotiables from Phase 1: never phrase anything as "allergen-free"/"safe" (only "the cook reports…"), always carry the cross-contact caution, and treat undeclared (legacy) dishes as unknown, never all-clear.
- [ ] **`ready_by_date` off-by-one across Pacific midnight** — 🅰️ ACCEPTED cosmetic (decided Aug 2026, surfaced by the full-code-review, PR #96). `orders.ready_by_date` is stamped at order CREATION in `app/checkout/actions.ts` from `pacificTodayIso()`. A **lead_time** order created just before Pacific midnight and paid just after promises a "ready by" date one day early. Deliberately NOT fixed: the window is ~2 min/day, "ready by" is an estimate not a hard deadline, only lead_time is affected (ready_now = today, preorder = fixed date), and zero money/data impact. The only real fix — recompute at payment confirmation — would push availability logic into the money-critical, idempotent `confirmPaidOrder` (shared by webhook + success page) for negligible payoff. Revisit only if buyers complain about a systematically-early date.
