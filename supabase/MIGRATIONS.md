...
**Status: 39 of 40 applied.** #40 (`add-order-refunded.sql`) is **NOT YET RUN** — a
marker column (`orders.refunded_at`) for the admin one-click refund; run it with/before
the paired code deploy (the refund action reads/writes it). #39 (`add-listing-availability.sql`) run by hand in the
Supabase SQL editor on 2026-08-03 (paired with the availability PR #87). #38
