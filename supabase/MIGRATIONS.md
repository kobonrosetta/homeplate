...
**Status: 33 of 34 applied.** #34 (`add-cook-archive.sql`) is **NOT YET RUN** — one
additive column (`cooks.archived_at`) powering the admin "archive" feature; run it
when the full-control admin PR deploys (until then, Archive buttons write to a column
that doesn't exist yet — deploy code + SQL together). #33 (`add-stripe-connect.sql`) applied +
