...
**Status: 32 of 33 applied.** #33 (`add-stripe-connect.sql`) is **NOT YET RUN** —
run it together with deploying the Stripe Connect PR (either order works: the
code's `stripe_ready` gate and `cook_stripe` reads simply find no rows / a
missing column only if the code deploys first, in which case checkout is closed
until the SQL runs — deliberate fail-closed). #31 (`harden-follows-orderitems.sql`) +
