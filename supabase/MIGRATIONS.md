...
**Status: 37 of 38 applied.** #38 (`harden-cook-private.sql`) is **NOT YET RUN** — a
column-guard trigger locking `permit_photo_path`; it is migration-LAST (deploy the
code that moves the permit-photo write to the service role first, then run the SQL).
#34–#37 were run by hand in the Supabase SQL editor on 2026-08-02 (verified during the
