...
**Status: 40 of 41 applied.** ⚠️ #41 (`add-waitlist.sql`) is **written but not yet run** —
paste it into the Supabase SQL editor before/with the waitlist PR deploy (the browse
empty-state form + admin count read/write the new table). #40 (`add-order-refunded.sql`) run by hand in the Supabase
SQL editor on 2026-08-04 (paired with the admin-refund PR #90). #39
(`add-listing-availability.sql`) run by hand in the Supabase SQL editor on 2026-08-03
(paired with the availability PR #87). #38
(`harden-cook-private.sql`) run
by hand in the Supabase SQL editor on 2026-08-02, after the code deploy that moved the
permit-photo write to the service role (migration-LAST order observed). #34–#37 were run by hand in
the Supabase SQL editor on 2026-08-02 (verified during the
targeted security review — #35, the order_items INSERT-policy drop, was re-run then to
confirm the buyer-inject-line-items hole is closed regardless of prior state; all four
are idempotent). #33 (`add-stripe-connect.sql`) applied +
...
| 38 | `harden-cook-private.sql` | Column-guard on `cook_private` (security review, Aug 2026). The "owner writes own private" policy is FOR ALL on the whole row and RLS filters rows not columns, so a cook's session could PATCH `cook_private.permit_photo_path` to point at ANOTHER cook's storage object — the admin would then be shown a stranger's permit (name/address) while reviewing this kitchen (no data reaches the attacker; the photo renders via signed URL to the admin only). Adds a BEFORE INSERT/UPDATE trigger `enforce_cook_private_rules` (same shape as `enforce_cook_cook_update_rules`): end-user sessions may write only `street_address`/`cdtfa_permit`/`pickup_location`/`updated_at`; `permit_photo_path` + `latitude`/`longitude` are server-managed. The sell wizard now writes the photo path via the service role. ⚠️ **Migration-LAST: deploy the code first** (the wizard's permit-photo write moved to the service role); running this while the old end-user-upsert code is live breaks permit-photo submit. | ✅ |
| 39 | `add-listing-availability.sql` | Dish availability/timing model — fixes buyers being able to order food that isn't planned to be made. Adds `listings.fulfillment_mode` ('ready_now'\|'lead_time'\|'preorder'), `listings.lead_days smallint` (0–14), `listings.ready_date date`, `listings.order_by date`; `orders.ready_by_date date` (the concrete date snapshotted at checkout, mirroring served_hot); and kitchen-DEFAULT `cooks.default_fulfillment_mode` + `cooks.default_lead_days` (new dishes inherit, per-dish override). Re-issues `enforce_cook_update_rules` (byte-for-byte #32 body) with the two default_* columns added to editable[] so a cook can set their kitchen default. Logic + Pacific date math in `lib/availability.ts` (unit-tested). Additive; no new tables; no RLS changes. Code (form control, checkout guard, buyer pills) reads the columns, so **run with/before the deploy** to avoid errors on the new columns. | ✅ |
| 40 | `add-order-refunded.sql` | Adds `orders.refunded_at timestamptz` — the marker behind the admin one-click refund (`app/admin/actions.ts refundOrder`, which calls Stripe with reverse_transfer + refund_application_fee so a destination-charge refund is always correct). It's a marker, not a status change: idempotent (blocks a second refund), and a refund on an already-completed order keeps its fulfilled history. Written only by the service-role admin action (bypasses the orders guard; not in any end-user allow-list). Additive; no RLS change. Code reads/writes the column, so **run with/before the deploy**. | ✅ |
| 41 | `add-waitlist.sql` | New `waitlist` table (buyer demand capture for the pre-launch / thin-supply browse page): `email` (unique), optional `zip`/`city`, server-set `source`, `created_at`. SERVICE-ROLE ONLY — RLS on, **zero policies**, exactly like `payouts` / `cook_stripe`, so the anon/authenticated keys can't read (scrape emails) or write it. The "Get notified" empty-state form posts to a server action (`app/browse/actions.ts joinWaitlist`) that upserts via the service role (ON CONFLICT DO NOTHING → re-submit is idempotent); admin count + `/admin/waitlist/export` CSV read via the service role. Additive; no other tables touched. Code reads/writes the table, so **run with/before the deploy** (until it exists the form insert errors, caught and shown as "try again"). ⚠️ **not yet applied.** | ⬜ |
